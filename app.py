...
# --- 🎨 CSS: บังคับ Layout ไม่ให้เลื่อนซ้อน และกำจัดที่ว่างด้านบน ---
st.markdown("""
    <style>
    /* กำจัดช่องว่างขาวด้านบนสุด */
    .block-container { padding-top: 1rem !important; padding-bottom: 0rem !important; }
    
    /* จัดการข้อความ Title */
    .main-title { font-size: 24px; font-weight: bold; color: white; margin-top: 5px; margin-bottom: 0px; }
    .sub-title { font-size: 13px; color: #94a3b8; margin-top: -5px; margin-bottom: 10px; }

    /* ตกแต่ง Metric Card */
    div[data-testid="stMetric"] { background: #1e293b; padding: 10px 15px; border-radius: 8px; border: 1px solid #334155; }
...
# --- 🔝 ส่วนหัว (Header): วางแบบเรียงลงมาเพื่อให้โลโก้ไม่โดนบัง ---
...
# แถวสำหรับตัวเลือก (วางไว้ระดับเดียวกับ Metrics เพื่อประหยัดพื้นที่ข้างบน)
col_sel1, col_sel2, col_m1, col_m2, col_m3, col_m4 = st.columns([1, 1, 1, 1, 1, 1])
...
if conn:
    try:
        ...
        # แสดง Metrics ในแถวเดียวกับตัวเลือก
        ...
        # --- 🏗️ ส่วนแสดงผลหลัก: [ซ้าย: แผนที่ + อันดับ] [ขวา: กราฟแนวโน้ม] ---
        ...
        with col_right:
            ...
            fig.update_layout(
                ...
                xaxis_title="เวลาอัปเดต",
                yaxis_title=s_thai_metric
            )
            st.plotly_chart(fig, use_container_width=True)
            
    except Exception as e: st.info(f"🔄 กำลังประมวลผลข้อมูล... ({e})")
else: st.error("⚠️ ไม่พบฐานข้อมูลใน data/ghg_data.db")
