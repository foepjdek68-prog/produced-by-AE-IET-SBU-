เข้าใจเลยครับ! ที่โค้ดเวอร์ชันก่อนหน้านี้แสดงผลเพี้ยนหรือไม่ตรงบลูพริ้นต์ เป็นเพราะธรรมชาติของ **Streamlit ไม่ยอมให้เราใช้คำสั่ง HTML `<div>` ไปห่อหุ้มกราฟของระบบอย่าง `st.plotly_chart` ตรง ๆ ครับ** พอเราเปิด-ปิดดิฟทิ้งไว้ มันเลยดันตัวกราฟหลุดกระจัดกระจายออกไปนอกกล่อง ผังหน้าจอเลยเบี้ยวและใหญ่เกินไป

ผมเลยทำการ **รีแฟกเตอร์ (Refactor) โครงสร้างใหม่หมดด้วยวิธีที่ถูกต้อง 100%** โดยหันมาใช้ระบบกล่องหุ้มแท้ของระบบอย่าง `st.container(border=True)` แล้วใช้ CSS ไปจับปรับแต่งสีพื้นหลังของกล่องแทน วิธีนี้จะทำให้ทั้งข้อความ กราฟเกจ และตาราง ยึดเกาะรวมกันอยู่ในกล่องการ์ดสีน้ำเงินเข้มอย่างสวยงาม ไม่หลุดสเกล และล็อกแถบเลือกไม่ให้คีย์บอร์ดเด้งได้เหมือนเดิมครับ

---

### 💻 โค้ดฉบับแก้ไขสมบูรณ์ (วางทับใน `app.py` แล้วรันได้ทันที)
