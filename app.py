...
def generate_clean_gauge(value, min_val, max_val, title_text, color):
    ...
    fig.update_layout(
        title={'text': title_text, 'font': {'size': 11, 'color': '#94a3b8', 'family': 'Inter', 'weight': 'bold'}, 'y': 0.9},
        height=75,
...
with g1:
    with st.container(border=True):
        st.plotly_chart(generate_clean_gauge(ctx["co2"], 350, 500, "1 🔵 ATMOSPHERIC CO₂ LEVEL"), use_container_width=True, config={'displayModeBar': False})
        st.markdown(f'<div style="text-align: center; font-size: 26px; font-weight: 800; color: #ffffff; margin-top: -10px;">{ctx["co2"]} <span style="font-size:14px; color:#94a3b8; font-weight:400;">ppm</span></div>', unsafe_allow_html=True)
...
with g2:
    with st.container(border=True):
        st.plotly_chart(generate_clean_gauge(ctx["temp"], 0, 4, "2 🟠 AV. TEMPERATURE ANOMALY"), use_container_width=True, config={'displayModeBar': False})
        st.markdown(f'<div style="text-align: center; font-size: 26px; font-weight: 800; color: #f97316; margin-top: -10px;">{ctx["temp_lbl"]}</div>', unsafe_allow_html=True)
...
with g3:
    with st.container(border=True):
        st.plotly_chart(generate_clean_gauge(ctx["aqi"], 0, 200, "3 🟡 AIR QUALITY INDEX (AQI)"), use_container_width=True, config={'displayModeBar': False})
        st.markdown(f'<div style="text-align: center; font-size: 26px; font-weight: 800; color: #ffffff; margin-top: -10px;">{ctx["aqi"]}</div>', unsafe_allow_html=True)
...
with m_left:
    with st.container(border=True):
        ...
        # เรนเดอร์แผนที่ความร้อนคลื่นวงกลมแบบสมูทสมจริง (Multi-peak Gaussian)
        grid_res = 100
        ...
        # ปักหมุดแลนด์มาร์กสำคัญด้วยแท็กสากลที่ปลอดภัยและไม่ทำให้แอปแครช
        ...
        # ส่วนแสดงผล Legend ด้านล่างแผนที่ความร้อนตรงปกอินโฟกราฟิก
        st.markdown("""
...
with m_right:
    with st.container(border=True):
        ...
        # กราฟล่าง: Combo Chart (Monthly PM2.5 & Temp)
...
with b_right:
    with st.container(border=True):
        ...
        # ตารางรายชื่อแม่น้ำพร้อมคลื่น SVG เรืองแสงและสถานะ Badge สวยงามตรงปก 100%
        river_html = """
